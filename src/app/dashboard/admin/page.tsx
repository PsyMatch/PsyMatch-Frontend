/* keep as client component; load heavy client-only nav without SSR to avoid prerender errors */
'use client';
import dynamic from 'next/dynamic';
const MenuNavegacionAdmin = dynamic(() => import('@/components/dashboard-admin/MenuNavegacionAdmin'), { ssr: false });
        ...
        <div className="min-h-screen w-full bg-gradient-to-br from-blue-50 to-indigo-100 relative overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
                    <FlowbiteLineChart data={sessionDataWeek} title="Sesiones de Terapia (por semana)" color="#5046E7" />
                    <FlowbiteBarChart data={appointmentsData} title="Citas Programadas (últimos 14 días)" color="#6366F1" />
                </div>

                {/* Gráficos de análisis */}
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
                    <FlowbitePieChart title="Páginas más visitadas" data={pagesData} />
                    <FlowbitePaymentsChart title="Pagos Semanales" data={paymentsData} />
                </div>
                ...
                <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-4 sm:p-6 lg:p-8">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 sm:mb-8 gap-4">
                        <div>
                            ...
                            <p className="text-gray-600 mt-2 text-base sm:text-lg">Administra pacientes, psicólogos y sus estados de verificación</p>
                        </div>
                        <div className="flex items-center space-x-4">
                            <div className="bg-[#5046E7] rounded-xl p-3 sm:p-4 shadow-lg">
                                <div className="flex items-center space-x-2 sm:space-x-3">
                                    <div className="w-3 h-3 sm:w-4 sm:h-4 bg-white rounded-full"></div>
                                    <span className="text-xs sm:text-sm font-semibold text-white">Total: {totalUsers} usuarios activos</span>
                                </div>
                            </div>
                        </div>
                    </div>
