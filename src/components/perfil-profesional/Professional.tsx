...
const Professional = ({ data }: { data: Profesional }) => {
    return (
        <div className="grid items-center w-full mt-10 md:grid-cols-[0.6fr_2fr_1fr] bg-white border-2 border-gray-200 h-56 mb-10 rounded-xl">
            ...
            <div className="flex flex-col justify-between h-40 text-start">
                <h3 className="text-xl font-bold text-black">{data.infoPersonal.nombreCompleto}</h3>
                <span className="text-xs font-semibold text-black">{data.infoPersonal.descripcion}</span>

                <div className="flex flex-row">
                    <span><strong>{data.infoPersonal.reseñas.porcentaje}</strong> reseñas</span>
                    <span>({data.infoPersonal.reseñas.cantidad}reseñas)</span>
                </div>
                ...
                <div className="flex flex-row gap-4 text-[10px] font-semibold text-gray-700">
                    <span>{data.infoPersonal.ubicación}</span>
                    <span>$ {data.infoPersonal.precio}/sesión</span>
                    <span className="text-green-700">Disponible {data.infoPersonal.disponiblidad}</span>
                </div>
            </div>
