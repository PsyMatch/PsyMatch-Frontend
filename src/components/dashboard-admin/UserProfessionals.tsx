            ...
            <div className="grid items-center w-[80%] h-10 grid-cols-2 gap-3 px-1 mt-6 bg-white rounded-md">
                <button
                    ...
                    className={`h-[80%] rounded-md transition-colors ${
                        filter === "Pendiente" ? "bg-violet-300" : "bg-gray-200 hover:bg-gray-100"
                    }`}
                ...
                <button
                    type="button"
                    className={`h-[80%] rounded-md transition-colors ${
                        filter === "Validado" ? "bg-violet-300" : "bg-gray-200 hover:bg-gray-100"
                    }`}
                        ...
                        <div key={psicologo.id}>
                            <div className="flex flex-col items-start justify-center w-full h-full gap-8 p-4 bg-gray-100 border-2 border-gray-200 rounded-md text-start">
                                ...
                                <div className="flex flex-row justify-between w-full">
                                    <div>
                                        <li><strong>DNI:</strong> {psicologo.dni}</li>
                                        <li><strong>Fecha de Nacimiento:</strong> {psicologo.birthdate}</li>
                                        <li><strong>Años de Experiencia:</strong> {psicologo.professional_experience}</li>
                                    </div>
                                    <div className="flex flex-col justify-end gap-3">
                                        {psicologo.verified === "Pendiente" &&
                                            <button  onClick={() => setAlerta(true)} className="px-4 py-1 rounded-md bg-violet-300 hover:bg-violet-200" type="button">Aceptar psicologo</button>                         
                                        }
                                        <button className="px-4 py-1 rounded-md bg-violet-300 hover:bg-violet-200" type="button" onClick={() => window.location.href = `/professionalProfile/${psicologo.id}`}>Ver perfil completo</button>
                                    </div>
                                </div>
                            </div>
                            {alerta && 
